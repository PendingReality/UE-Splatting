	...
	FGaussianGlobalAccumulator* Accumulator)
{
	...
	FMatrix CurTranslatedVP = View.ViewMatrices.GetTranslatedViewMatrix() * View.ViewMatrices.GetViewToClipNoAA();
	...
	bool bUseLODRendering)
{
	...
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FGaussianSplatCalcViewDataCS shader not valid"));
		return;
	}
	...
	Parameters.WorldToClip = FMatrix44f(View.ViewMatrices.GetWorldToView() * View.ViewMatrices.GetViewToClipNoAA());
	...
	int32 SplatCount)
{
	...
	if (!ComputeShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FGaussianSplatCalcDistancesCS shader not valid"));
		return;
	}
	...
	int32 SplatCount)
{
	...
	{
		UE_LOG(LogUESplatting, Warning, TEXT("Radix sort shaders not valid"));
		return;
	}
	...
	bool bUseLODRendering)
{
	...
	if (!ComputeShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FCompactSplatsCS shader not valid"));
		return;
	}
	...
	FGaussianSplatGPUResources* GPUResources)
{
	...
	if (!ComputeShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FPrepareIndirectArgsCS shader not valid"));
		return;
	}
	...
	int32 TotalSplatCount)
{
	...
	if (!CountShader.IsValid() || !PrefixSumShader.IsValid() ||
		!DigitPrefixSumShader.IsValid() || !ScatterShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("Radix sort global shaders not valid"));
		return;
	}
	...
	int32 ProxyIndex)
{
	...
	if (!ComputeShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FGatherVisibleCountsCS shader not valid"));
		return;
	}
	...
	uint32 MaxRenderBudget)
{
	...
	if (!ComputeShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FPrefixSumVisibleCountsCS shader not valid"));
		return;
	}
	...
	FGaussianGlobalAccumulator* GlobalAccumulator)
{
	...
	if (!CountShader.IsValid() || !PrefixSumShader.IsValid() ||
		!DigitPrefixSumShader.IsValid() || !ScatterShader.IsValid())
	{
		UE_LOG(LogUESplatting, Warning, TEXT("Radix sort global indirect shaders not valid"));
		return;
	}
	...
	bool bUseLODRendering)
{
	...
	{
		UE_LOG(LogUESplatting, Warning, TEXT("Cluster culling shaders not valid"));
		return 0;
	}
	...
	// Step 2: Run cluster culling with LOD selection
	{
		...
		ExtractFrustumPlanes(View.ViewMatrices.GetWorldToClip(), FrustumPlanes);
		...
		CullingParams.WorldToClip = FMatrix44f(View.ViewMatrices.GetWorldToClip());
	...
	// Return the dispatched leaf count; the exact visible count remains GPU-resident.
