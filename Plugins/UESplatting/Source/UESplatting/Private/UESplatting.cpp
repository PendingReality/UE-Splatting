...
#define LOCTEXT_NAMESPACE "FUESplattingModule"

DEFINE_LOG_CATEGORY(LogUESplatting);
...
/** Show custom Cluster LOD debug visualization. */
TAutoConsoleVariable<int32> CVarShowClusterBounds(
	TEXT("gs.ShowClusterBounds"),
	0,
	TEXT("Debug visualization for UESplatting Cluster LOD.\n")
	TEXT("When enabled, shows cluster colors on a black background.\n")
	TEXT(" 0: Off (default)\n")
 ...
 *  this budget (after Cluster LOD compaction), excess splats are simply not rendered.
...
TAutoConsoleVariable<int32> CVarDebugForceLODLevel(
	...
	-1,
	TEXT("Force rendering of a specific LOD level for debugging (only affects Cluster LOD assets).\n")
...
void FUESplattingModule::StartupModule()
{
	// Register the shader directory so we can use our custom shaders
	const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("UESplatting"));
	checkf(Plugin.IsValid(), TEXT("UESplatting plugin descriptor could not be found."));
	const FString PluginShaderDir = FPaths::Combine(Plugin->GetBaseDir(), TEXT("Shaders"));
	AddShaderSourceDirectoryMapping(TEXT("/Plugin/UESplatting"), PluginShaderDir);
	...
	PostOpaqueRenderDelegateHandle = GetRendererModuleRef().RegisterPostOpaqueRenderDelegate(
		FPostOpaqueRenderDelegate::CreateRaw(this, &FUESplattingModule::OnPostOpaqueRender_RenderThread));
	...
	PostEngineInitDelegateHandle = FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FUESplattingModule::OnPostEngineInit);

	UE_LOG(LogUESplatting, Log, TEXT("UESplatting runtime module started. Shader directory: %s"), *PluginShaderDir);
}

void FUESplattingModule::OnPostEngineInit()
{
	// Skip view extension creation during cooking/commandlet — GEngine is null in those contexts
	if (IsRunningCommandlet() || !GEngine)
	{
		UE_LOG(LogUESplatting, Log, TEXT("UESplatting: Skipping ViewExtension creation (commandlet/cook mode)"));
		return;
	}
	...
	{
		UE_LOG(LogUESplatting, Error, TEXT("UESplatting: Failed to create ViewExtension!"));
	}
	else
	{
		UE_LOG(LogUESplatting, Log, TEXT("UESplatting: ViewExtension created successfully (deferred init)"));
	}
}

void FUESplattingModule::OnPostOpaqueRender_RenderThread(FPostOpaqueRenderParameters& Parameters)
{
	...
	//------------------------------------------------------------------
		...
		FMatrix CurrentVP = SceneView->ViewMatrices.GetWorldToView() * SceneView->ViewMatrices.GetViewToClipNoAA();
...
void FUESplattingModule::ShutdownModule()
{
...
FUESplattingModule& FUESplattingModule::Get()
{
	return FModuleManager::LoadModuleChecked<FUESplattingModule>("UESplatting");
}

bool FUESplattingModule::IsAvailable()
{
	return FModuleManager::Get().IsModuleLoaded("UESplatting");
}

#undef LOCTEXT_NAMESPACE

IMPLEMENT_MODULE(FUESplattingModule, UESplatting)
