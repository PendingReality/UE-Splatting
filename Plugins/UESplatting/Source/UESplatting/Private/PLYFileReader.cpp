...
bool FPLYFileReader::ReadPLYFile(const FString& FilePath, TArray<FGaussianSplatData>& OutSplats, FString& OutError, int32* OutSHBands)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("PLY file size: %lld bytes (%.2f GB)"), FileSize, FileSize / (1024.0 * 1024.0 * 1024.0));
	...
	UE_LOG(LogUESplatting, Log, TEXT("PLY Header parsed: %d vertices, %d bytes per vertex, data at offset %lld"),
	...
	if (OutSHBands)
	{
		...
		UE_LOG(LogUESplatting, Log, TEXT("PLYFileReader: Detected SH bands = %d"), *OutSHBands);
	}
	...
	UE_LOG(LogUESplatting, Log, TEXT("Successfully read %d splats from PLY file"), OutSplats.Num());
	return true;
}
...
bool FPLYFileReader::ReadVertexData(IFileHandle* FileHandle, const FPLYHeader& Header, TArray<FGaussianSplatData>& OutSplats, FString& OutError)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("PLYFileReader: VertexCount=%d, SH CoeffsPerChannel=%d"),
	...
	while (VerticesRemaining > 0)
	{
		...
		{
			UE_LOG(LogUESplatting, Log, TEXT("  Reading PLY vertices: %d / %d (%.0f%%)"),
