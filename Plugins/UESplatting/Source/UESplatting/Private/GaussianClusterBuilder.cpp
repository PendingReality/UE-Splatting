	...
	const FBuildSettings& Settings)
{
	...
	if (NumSplats == 0)
	{
		UE_LOG(LogUESplatting, Warning, TEXT("FGaussianClusterBuilder: No splats to cluster"));
		return false;
	}

	UE_LOG(LogUESplatting, Log, TEXT("FGaussianClusterBuilder: Building cluster hierarchy for %d splats"), NumSplats);
	...
	UE_LOG(LogUESplatting, Log, TEXT("  Global bounds: Min(%s) Max(%s)"),
	...
	UE_LOG(LogUESplatting, Log, TEXT("  Sorted splats by Morton code"));
	...
	UE_LOG(LogUESplatting, Log, TEXT("  Created %d leaf clusters"), LeafClusters.Num());
	...
	while (CurrentLevel.Num() > 1)
	{
		...
		UE_LOG(LogUESplatting, Log, TEXT("  LOD Level %d: %d clusters"), CurrentLODLevel, ParentLevel.Num());
	...
	UE_LOG(LogUESplatting, Log, TEXT("FGaussianClusterBuilder: Hierarchy complete"));
	UE_LOG(LogUESplatting, Log, TEXT("  Total clusters: %d"), OutHierarchy.Clusters.Num());
	UE_LOG(LogUESplatting, Log, TEXT("  LOD levels: %d"), OutHierarchy.NumLODLevels);
	UE_LOG(LogUESplatting, Log, TEXT("  Root cluster index: %d"), OutHierarchy.RootClusterIndex);
	...
	if (Settings.bGenerateLOD && OutHierarchy.NumLODLevels > 1)
	{
		...
		UE_LOG(LogUESplatting, Log, TEXT("  Generated %d LOD splats"), OutHierarchy.TotalLODSplatCount);
		...
		for (uint32 Level = 1; Level <= OutHierarchy.NumLODLevels; Level++)
		{
			...
			if (Count > 0)
			{
				UE_LOG(LogUESplatting, Log, TEXT("  LOD Level %d: MaxError min=%.2f avg=%.2f max=%.2f, Radius min=%.2f avg=%.2f max=%.2f (%d clusters)"),
	...
	int32 ReductionRatio)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("GenerateLODSplats: Generating LOD splats with reduction ratio %d"), ReductionRatio);
	...
	UE_LOG(LogUESplatting, Log, TEXT("GenerateLODSplats: Generated %d LOD splats"), Hierarchy.TotalLODSplatCount);
}
