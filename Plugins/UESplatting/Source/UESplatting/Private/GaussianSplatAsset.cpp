...
TSharedPtr<FGaussianSplatRenderData> UGaussianSplatAsset::GetOrCreateRenderData()
{
	...
	{
		UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatRenderData: Reusing existing shared data for asset '%s'"),
			*GetName());
		return RenderData;
	}
...
void UGaussianSplatAsset::InvalidateRenderDataAndNotify()
{
	RenderData.Reset();
...
void UGaussianSplatAsset::Serialize(FArchive& Ar)
{
	...
	{
		UE_LOG(LogUESplatting, Error, TEXT("GaussianSplatAsset: Incompatible asset format (Magic=0x%08X, Version=%d). Please reimport the asset."), Magic, Version);
		return;
	}
...
void UGaussianSplatAsset::PostLoad()
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("GaussianSplatAsset::PostLoad - SplatCount=%d, ColorTextureBulkData.Size=%lld, Width=%d, Height=%d"),
	...
	if (ColorDataSize > 0 && ColorTextureWidth > 0 && ColorTextureHeight > 0)
	{
		...
		UE_LOG(LogUESplatting, Log, TEXT("GaussianSplatAsset::PostLoad - ColorTexture recreated successfully"));
	}
	else
	{
		UE_LOG(LogUESplatting, Warning, TEXT("GaussianSplatAsset::PostLoad - No ColorTextureBulkData to restore (might be old asset format)"));
	}
	...
	if (ThumbnailData.Num() > 0 && ThumbnailSize > 0)
	{
		...
		UE_LOG(LogUESplatting, Log, TEXT("GaussianSplatAsset::PostLoad - ThumbnailTexture recreated successfully"));
	}
#endif
}

#if WITH_EDITOR
void UGaussianSplatAsset::SetReimportSourceFilePath(const FString& NewSourceFilePath)
{
...
void UGaussianSplatAsset::InitializeFromSplatData(const TArray<FGaussianSplatData>& InSplats, EGaussianQualityLevel InQuality)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("GaussianSplatAsset: Initialized with %d splats, memory: %lld bytes"),
...
void UGaussianSplatAsset::CreateColorTextureData(const TArray<FGaussianSplatData>& InSplats)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("CreateColorTextureData: Creating %dx%d data for %d splats"),
	...
	UE_LOG(LogUESplatting, Log, TEXT("CreateColorTextureData: Stored %d bytes of pixel data"), NumBytes);
}
...
void UGaussianSplatAsset::CreateColorTextureFromData()
{
	...
	{
		UE_LOG(LogUESplatting, Warning, TEXT("CreateColorTextureFromData: No data to create texture from"));
		return;
	}

	UE_LOG(LogUESplatting, Log, TEXT("CreateColorTextureFromData: Creating %dx%d texture from stored bulk data"),
	...
	UE_LOG(LogUESplatting, Log, TEXT("CreateColorTextureFromData: Texture created and resource updated"));
}
...
void UGaussianSplatAsset::CompressSH(const TArray<FGaussianSplatData>& InSplats)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("CompressSH: SHBands=%d, SplatCount=%d, InSplats.Num()=%d, NonZeroSH(first100)=%d, MaxSHValue=%.6f"),
	...
	// Float16 is the supported higher-order SH storage format.
...
bool UGaussianSplatAsset::BuildNaniteClusterHierarchy()
{
	...
	{
		UE_LOG(LogUESplatting, Error, TEXT("BuildNaniteClusterHierarchy: Source file path is empty"));
		return false;
	}

	if (!FPaths::FileExists(SourceFilePath))
	{
		UE_LOG(LogUESplatting, Error, TEXT("BuildNaniteClusterHierarchy: Source file not found: %s"), *SourceFilePath);
		return false;
	}

	FScopedSlowTask SlowTask(100.0f, FText::FromString(TEXT("Building Cluster LOD Hierarchy...")));
	SlowTask.MakeDialog(true);

	// Step 1: Re-read the normalized source splat data.
	SlowTask.EnterProgressFrame(30.0f, FText::FromString(TEXT("Decoding source splat file...")));

	FSplatDecodeResult Decoded;
	FString ErrorMessage;

	if (!FGaussianSplatDecoderRegistry::DecodeFile(SourceFilePath, Decoded, ErrorMessage))
	{
		UE_LOG(LogUESplatting, Error, TEXT("BuildNaniteClusterHierarchy: Failed to decode source splat file: %s"), *ErrorMessage);
		return false;
	}

	TArray<FGaussianSplatData> SplatData = MoveTemp(Decoded.Splats);

	// Keep SH band metadata consistent with the decoded source before reinitializing
	// (CompressSH reads SHBands). Behaviour-preserving for PLY; correct for any format.
	SHBands = Decoded.SHBands;

	UE_LOG(LogUESplatting, Log, TEXT("BuildNaniteClusterHierarchy: Decoded %d source splats"), SplatData.Num());
	...
	{
		UE_LOG(LogUESplatting, Error, TEXT("BuildNaniteClusterHierarchy: Failed to build cluster hierarchy"));
		return false;
	}

	UE_LOG(LogUESplatting, Log, TEXT("BuildNaniteClusterHierarchy: Built %d clusters, %d LOD levels, %d LOD splats"),
	...
	if (NewClusterHierarchy.LODSplats.Num() > 0)
	{
		...
		UE_LOG(LogUESplatting, Log, TEXT("BuildNaniteClusterHierarchy: Unified buffer: %d original + %d LOD = %d total splats"),
	...
	// Enable the custom cluster LOD path (serialized field retains its legacy name).
	bEnableNanite = true;

	// Mark package dirty
	MarkPackageDirty();

	UE_LOG(LogUESplatting, Log, TEXT("BuildNaniteClusterHierarchy: Successfully built Cluster LOD hierarchy. Total splats: %d, Clusters: %d"),
		SplatCount, ClusterHierarchy.Clusters.Num());

	// Invalidate shared render data so it will be recreated with new cluster data
	InvalidateRenderDataAndNotify();
...
void UGaussianSplatAsset::ClearNaniteClusterHierarchy()
{
	...
	{
		UE_LOG(LogUESplatting, Warning, TEXT("ClearNaniteClusterHierarchy: Cluster LOD is not enabled"));
		return;
	}
	...
	{
		// Re-read the original source file to get clean splat data without LOD splats.
		if (!SourceFilePath.IsEmpty() && FPaths::FileExists(SourceFilePath))
		{
			...
			if (FGaussianSplatDecoderRegistry::DecodeFile(SourceFilePath, Decoded, ErrorMessage))
			{
				// Reinitialize with original splats only (no LOD); keep SH metadata consistent
				SHBands = Decoded.SHBands;
				InitializeFromSplatData(Decoded.Splats, ImportQuality);
				UE_LOG(LogUESplatting, Log, TEXT("ClearNaniteClusterHierarchy: Restored %d original splats"), Decoded.Splats.Num());
			}
			else
			{
				UE_LOG(LogUESplatting, Warning, TEXT("ClearNaniteClusterHierarchy: Could not re-read source file, keeping current splat data"));
			}
		}
		else
		{
			UE_LOG(LogUESplatting, Warning, TEXT("ClearNaniteClusterHierarchy: Source file not found, keeping current splat data (includes LOD splats)"));
		}
	}
	...
	UE_LOG(LogUESplatting, Log, TEXT("ClearNaniteClusterHierarchy: Cleared Cluster LOD hierarchy"));
...
void UGaussianSplatAsset::GenerateThumbnail(const TArray<FGaussianSplatData>& InSplats)
{
	...
	UE_LOG(LogUESplatting, Log, TEXT("GaussianSplatAsset: Thumbnail generated (%dx%d)"), ThumbnailSize, ThumbnailSize);
}
