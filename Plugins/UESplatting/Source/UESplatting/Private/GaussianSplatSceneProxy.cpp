...
void FGaussianSplatGPUResources::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created %d per-instance buffers (shared data: %s)"),
...
void FGaussianSplatGPUResources::CreateDummyWhiteTexture(FRHICommandListBase& RHICmdList)
{
	...
	DummyWhiteTexture = RHICmdList.CreateTexture(TextureDesc);

	// Fill with white color
	uint32 WhitePixel = 0xFFFFFFFF;  // RGBA = (255, 255, 255, 255)
	FUpdateTextureRegion2D Region(0, 0, 0, 0, 1, 1);
	RHICmdList.UpdateTexture2D(DummyWhiteTexture, 0, Region, sizeof(uint32), reinterpret_cast<const uint8*>(&WhitePixel));
...
void FGaussianSplatGPUResources::CreatePerInstanceBuffers(FRHICommandListBase& RHICmdList)
{
	...
	// Total: 20 bytes, but we use 32 bytes for alignment
	{
		...
		UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created indirect draw buffer"));
	}
	...
	// One bit per cluster, rounded up to uint32 boundary
	{
		...
		UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created cluster visibility bitmap (%d bytes for %d clusters)"), BitmapSize, ClusterCount);
	}
	...
	// One entry per leaf cluster, stores which cluster ID is selected based on LOD
	{
		...
		UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created selected cluster buffer (%d bytes for %d leaf clusters)"), BufferSize, LeafClusterCount);
	}
	...
	UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created LOD cluster tracking buffers"));
	...
	UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created splat compaction buffers for %d total splats"), TotalSplatCount);
	UE_LOG(LogUESplatting, Verbose, TEXT("GaussianSplatGPUResources: Created cluster buffers for %d clusters"), ClusterCount);
}
