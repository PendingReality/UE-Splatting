...
// Delegate fired when asset data changes (for example, Cluster LOD rebuilt or cleared)
...
/**
 * Asset containing normalized Gaussian Splatting data loaded by the decoder registry
 * Stores compressed splat data optimized for GPU rendering
 */
UCLASS(BlueprintType, hidecategories = Object)
class UESPLATTING_API UGaussianSplatAsset : public UObject
{
	GENERATED_BODY()
...
public:
	...
	/** Check if UESplatting's custom cluster LOD hierarchy is enabled for this asset. */
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Cluster LOD", meta = (DisplayName = "Is Cluster LOD Enabled"))
	bool IsNaniteEnabled() const { return bEnableNanite; }

#if WITH_EDITOR
	/** Update or clear the source path used by editor reimport and Cluster LOD rebuilds. */
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Import")
	void SetReimportSourceFilePath(const FString& NewSourceFilePath);
	...
	/**
	 * Build UESplatting's custom cluster LOD hierarchy from the source splat file.
	 * This is not Unreal Engine Nanite.
	 * @return True if successful, false if source file not found or build failed
	 */
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Cluster LOD", meta = (DisplayName = "Build Cluster LOD Hierarchy"))
	bool BuildNaniteClusterHierarchy();

	/**
	 * Clear the custom cluster LOD hierarchy to reduce asset size
	 * Removes cluster data and LOD splats
	 */
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Cluster LOD", meta = (DisplayName = "Clear Cluster LOD Hierarchy"))
	void ClearNaniteClusterHierarchy();

	/**
	 * Set custom cluster LOD enabled state (internal/serialized compatibility name)
	 */
	...
	UFUNCTION(BlueprintCallable, Category = "Gaussian Splatting|Cluster LOD")
	...
	/** Delegate fired when asset data changes (Cluster LOD, reimport, etc.) */
	...
	/** Drop cached CPU/GPU render data and recreate any registered component proxies. */
	void InvalidateRenderDataAndNotify();
...
public:
	...
	/** Legacy serialized field retained for asset compatibility; current storage is lossless-position/half-SH. */
	UPROPERTY()
	EGaussianQualityLevel ImportQuality = EGaussianQualityLevel::Medium;

	/**
	 * Whether UESplatting's custom cluster LOD and culling is enabled for this asset.
	 * This is a plugin-owned hierarchy, not Unreal Engine Nanite.
	 */
	UPROPERTY(VisibleAnywhere, Category = "Cluster LOD", meta = (DisplayName = "Cluster LOD Enabled"))
	bool bEnableNanite = false;

	/**
	 * Hierarchical cluster structure for UESplatting LOD and culling
	 ...
	 * When Cluster LOD is enabled, SplatCount includes both original + LOD splats
	 */
	UPROPERTY()
...
public:
	 ...
	 * @param InSplats Normalized raw splat data from a registered decoder
	 * @param InQuality Legacy storage selection retained for asset compatibility
	 */
